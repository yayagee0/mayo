<script lang="ts">
	...
	function getPollResults() {
		...
		return data.options.map((option, index) => {
			...
			return { option, votes, percentage: Math.round(percentage) };
		});
	}
	...
	async function deletePost() {
		...
		try {
			deleting = true;
			const { error } = await supabase.from('items').delete().eq('id', post.id);
			if (error) throw error;
...
<div class="bg-white rounded-lg border border-gray-200 p-4" style="margin-left: {level * 1.5}rem">
	...
	<!-- Content -->
	{#if post.body && post.body.trim() && !post.body.includes("youtube.com")}
		<div class="mb-3">
	...
	<div class="flex items-center gap-4 pt-3 border-t border-gray-100">
		...
		{#if showComments && level < 2}
			<button onclick={() => showReplyComposer = !showReplyComposer}
		...
		<button class="flex items-center gap-2 text-sm text-gray-500 hover:text-gray-700 transition-colors rounded px-2 py-1 focus:outline-none focus:ring-2 focus:ring-primary-500">
	...
	{#if showDeleteConfirm}
		<div class="mt-4 p-3 bg-red-50 border border-red-200 rounded-lg">
			<p class="text-sm text-red-800 mb-3">Are you sure you want to delete this post? This action cannot be undone.</p>
			<div class="flex gap-2">
				...
				<button onclick={() => showDeleteConfirm = false} class="btn btn-sm btn-secondary">
					Cancel
				</button>
			</div>
		</div>
	...
	{#if showReplyComposer}
		<div class="mt-4 p-3 bg-gray-50 rounded-lg">
			<textarea bind:value={replyContent} placeholder="Write a reply..." rows="3"
				class="w-full p-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-primary-500 resize-none text-sm"></textarea>
			<div class="flex justify-end gap-2 mt-2">
				<button onclick={() => showReplyComposer = false}
					class="px-3 py-1 text-sm text-gray-600 hover:text-gray-800">Cancel</button>
				<button onclick={submitReply} disabled={!replyContent.trim() || submittingReply}
