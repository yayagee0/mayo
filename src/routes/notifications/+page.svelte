<script lang="ts">
	...
	async function loadNotifications() {
		...
		try {
			...
			// Fetch interactions (likes, poll votes, etc.)
			const { data: interactions, error: interactionsError } = await supabase
				.from('interactions')
				.select('*')
				.in('item_id', userItems.map(i => i.id))
			...
			const { data: comments, error: commentsError } = await supabase
				...
				.in('parent_id', userItems.map(i => i.id))
				.order('created_at', { ascending: false })
				.limit(20);
			...
			// Convert interactions to notifications
			...
			// Convert comments to notifications
			...
			newNotifications
				...
				.forEach(n => notificationStore.add({
					type: n.type,
					title: n.title,
					message: n.message,
					data: n.data
...
<div class="min-h-screen bg-gray-50 pb-20">
		...
		<div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
			<div class="flex justify-between items-center h-16">
				...
				<div class="flex gap-2">
					{#if $notificationStore.some(n => !n.read)}
						<button
							type="button"
	...
	<main class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
		...
		{:else if $notificationStore.length === 0}
				...
				{#each $notificationStore as notification (notification.id)}
					...
					>
						<div class="flex items-start gap-3">
							<ActivityIcon class="w-5 h-5 mt-0.5 flex-shrink-0" aria-hidden="true" />
							<div class="flex-1">
